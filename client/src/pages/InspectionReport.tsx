                            ...
                            return (
                              <div key={instanceName} className="space-y-2 mb-6">
                                ...
                                {section.fields.map((field, fieldIdx) => {
                                  ...
                                  // Collect photos for this field
                                  if (entry?.photos && entry.photos.length > 0) {
                                    entry.photos.forEach((photo: string, photoIdx: number) => {
                                      instancePhotos.push({
                                        field,
                                        entry,
                                        fieldIdx,
                                ...
                                {/* Render all photos for this instance after all fields */}
                                {instancePhotos.length > 0 && (
                                  <div className="py-4 px-4 bg-muted/20 border-t-2 border-primary/20 mt-4">
                                    <div className="mb-4">
                                    ...
                                    {/* Action Buttons for instance (if any entry has maintenance capability) */}
                                    {!editMode && instancePhotos.length > 0 && inspection.propertyId && (
                                      <div className="flex gap-2 flex-wrap mt-4 pt-3 border-t no-print">
                                        {inspection.type === 'check_out' && inspection.propertyId && (
                                          <Button
                                            ...
                                            size="sm"
                                            onClick={() => {
                                              // Create comparison for the first field with photos
                                              const firstPhoto = instancePhotos[0];
                                              if (firstPhoto) {
                                                autoCreateComparisonMutation.mutate({
                                                  inspectionId: id!,
                                                  fieldKey: firstPhoto.field.id || firstPhoto.field.key || firstPhoto.field.label
                                                });
                                              }
                                            }}
