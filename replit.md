...
- **In-Inspection Quick Actions**: Contextual quick-add workflow enabling BTR operators to add assets and log maintenance without leaving the inspection capture page. Features include:
  - **Floating Action Button (FAB)**: Bottom-right positioned button with brand colors for thumb-friendly mobile access
  ...
  - **Audit Trail Linking**: All quick-added assets and maintenance requests automatically link to source inspection via `inspectionId` and `inspectionEntryId` fields for compliance tracking
  - **Offline Support**: Discriminated union queue system supports inspection_entry, asset, and maintenance payloads with dedicated sync methods and offline deduplication via unique offlineId tokens
  - **Backend APIs**: POST /api/asset-inventory/quick and POST /api/maintenance/quick endpoints with Zod validation, organization isolation, and offline deduplication support
  - **Context Preservation**: Quick actions minimize context switching—operators can document findings immediately without losing inspection progress or navigating away
