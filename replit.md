...
- **Excel Bulk Import System**: Comprehensive bulk data import facility using SheetJS (xlsx) for parsing Excel files. Supports importing Blocks, Properties, Tenants, and Assets with downloadable templates, two-step validation/commit workflow, row-level error reporting, and organization-scoped data isolation. Accessible via Settings > Bulk Import in the sidebar.
- **Tenant Community Discussions**: Block-scoped community feature allowing tenants to create discussion groups, post threads, and reply to discussions. Includes versioned community rules that tenants must accept before participating, operator approval workflow for new groups, content flagging system, and comprehensive moderation tools. Tenants access via tenant portal "Community" page; operators manage via "Community" in sidebar with approval queue, flagged content review, moderation log, and tenant blocking capability (blocked tenants cannot create threads or post replies). AI-generated discussion group banners using DALL-E 3 with organization logo compositing in top-right corner; banner colors complement the Inspect360 brand palette (teal/cyan).
